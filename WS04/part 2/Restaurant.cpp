...
namespace seneca {
  ...
  Restaurant::Restaurant(const Reservation* reservations[], size_t cnt) : m_cnt(cnt) {
    m_reserve = new Reservation*[m_cnt];
    for (size_t i = 0; i < m_cnt; ++i) m_reserve[i] = new Reservation(*reservations[i]);
  }

  Restaurant::Restaurant(const Restaurant& uneAutre) : m_cnt(uneAutre.m_cnt) {
    m_reserve = new Reservation*[m_cnt];
    for (size_t i = 0; i < m_cnt; ++i) m_reserve[i] = new Reservation(*uneAutre.m_reserve[i]);
  }
