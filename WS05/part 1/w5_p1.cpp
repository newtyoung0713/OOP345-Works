...
int main(int argc, char** argv)
{
    ...
    if (argc == 2) {
        ...
        size_t cnt = 0;
        do {
            std::getline(import, strBook);
            if (import) if (strBook[0] != '#') library[++cnt] = seneca::Book(strBook);
        } while (import);
        import.close();
    }
    else
    ...
    for (auto& bk : library) std::cout << bk << std::endl;
    ...
    for (auto& bk : library) updatePrice(bk);
